...
@Component({
    ...
    styleUrls: ['./package-units.component.css']
})
export class PackageUnitsComponent implements OnInit {
    ...
    getSavedFilterdata() {
        ...
        let filterData: any = localStorage.getItem('packagesUnitFilterData');
        if (filterData) {
            filterData = JSON.parse(filterData);
            if (filterData.page) {
    ...
    ngOnChanges() {
        if (this.packageDetails) {
            ...
            // this.getFilterModelList(this.packageDetails.project_id);
        }
    }
    ...
    saveFilter() {
        ...
        localStorage.setItem('packagesUnitFilterData', JSON.stringify(data));
    }
