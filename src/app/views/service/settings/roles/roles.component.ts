...
export class RolesComponent implements OnInit {
    ...
    getSavedFilterdata() {
        ...
        let filterData: any = localStorage.getItem('serviceSettingsRolesFilterData');
        if (filterData) {
    ...
    getItemslist() {
        ...
        this.webService.get(url).subscribe((response: any) => {
            ...
            if (response.status == 1) {
                this.itemsList = [];
                this.itemsList = (response.results && response.results[0] && response.results[0].roles) ? response.results[0].roles : [];
    ...
    saveFilter() {
        ...
        localStorage.setItem('serviceSettingsRolesFilterData', JSON.stringify(data));
    }
