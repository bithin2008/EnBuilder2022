...
@Component({
    selector: 'app-options-model',
    templateUrl: './options-model.component.html',
    styleUrls: ['./options-model.component.css']
})
export class OptionsModelComponent implements OnInit {
    ...
    getSavedFilterdata() {
        ...
        let filterData: any = localStorage.getItem('personalOptionsModelFilterData');
        if (filterData) {
            filterData = JSON.parse(filterData);
            if (filterData.page) {
    ...
    saveFilter() {
        ...
        localStorage.setItem('personalOptionsModelFilterData', JSON.stringify(data));
    }
