...
@Component({
    selector: 'app-options-unit',
    templateUrl: './options-unit.component.html',
    styleUrls: ['./options-unit.component.css']
})
export class OptionsUnitComponent implements OnInit {
    ...
    ngOnChanges() {
        if (this.optionDetailsObj) {
            ...
            // this.getFilterModelList(this.optionDetailsObj.project_id);
        }
    }

    getSavedFilterdata() {

        let filterData: any = localStorage.getItem('personalOptionsUnitFilterData');
        if (filterData) {
            filterData = JSON.parse(filterData);
            if (filterData.page) {
    ...
    getModelsList() {
        ...
        let data: any = {};
        data.project_id = this.optionDetailsObj.project_id;
    ...
    //             if (response.status == 1) {
    //                 this.filterModelList = response.results;
    ...
    getOptionsTypeList() {
        let url = `package-center/options-of-model-types?project_id=${this.optionDetailsObj.project_id}`;
        this.typeOptions = [];
        this.spinnerService.show();
        this.webService.get(url).subscribe((response: any) => {
            this.spinnerService.hide();
    ...
    getOptionsCollectionList() {
        let url = `package-center/options-of-model-collections?project_id=${this.optionDetailsObj.project_id}`;
        this.spinnerService.show();
    ...
    saveFilter() {
        ...
        localStorage.setItem('personalOptionsUnitFilterData', JSON.stringify(data));
    }
    ...
    getSeletedUnitList() {
        ...
        data.purpose = 'list';
        data.project_id = this.optionDetailsObj.project_id;
        data.subtype = 'PERSONALIZATION_OPTION';
        data.page = this.page;
