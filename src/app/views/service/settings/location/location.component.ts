...
export class LocationComponent implements OnInit {
    ...
    getSavedFilterdata() {
        ...
        let filterData: any = localStorage.getItem('serviceSettingsLocationsFilterData');
        if (filterData) {
    ...
    getItemslist() {
        this.saveFilter();
        this.spinnerService.show();
        let url = `service/project-settings?type=PROJECT-ISSUE-LOCATION&page=${this.page}&pageSize=${this.pageSize}`;
        if (this.sortedtby)
    ...
    saveFilter() {
        ...
        localStorage.setItem('serviceSettingsLocationsFilterData', JSON.stringify(data));
    }
    ...
    onStepChange(data) {
        this.spinnerService.show();
        let url = `service/rearrange-project-settings`;
        this.webService.post(url, data).subscribe((response: any) => {
            this.spinnerService.hide();
            if (response.status == 1) {
                this.toastr.success(response.message, 'Success');
